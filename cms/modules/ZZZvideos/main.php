...
$db->Query("SELECT COUNT(*) FROM $mainSession WHERE rel = '{$_REQUEST['rel']}' AND id_rel = '{$_REQUEST['id_rel']}'");
...
if ($totalListRows > 0) {
	$db->Query("SELECT $mainSession.* 
					...
					." WHERE rel = '{$_REQUEST['rel']}' AND id_rel = '{$_REQUEST['id_rel']}'";
	?>
	...
	<form name="multipleExclusionForm" id="multipleExclusionForm" method="post" enctype="multipart/form-data">
		<table width="100%" border="0" cellpadding="5" cellspacing="0" id="listData">
		  <tr>
			...
			<th><?=AdminKernel::getThOrderLabel("Legenda", "$mainSession.legenda");?></th>
			<th><?=AdminKernel::getThOrderLabel("Link", "$mainSession.youtube_url");?></th>
			<th width="110"> </th>
		  </tr>
		<?
		  $trId = 0;
		...
		?>
			 <tr class="<?="listData_tr".($trId%2)?>" ondblclick="window.location='<?=AdminKernel::getAltHref($row[$primaryKey]);?>';" onmouseover="$(this).toggleClass('listData_highlight');" onmouseout="$(this).toggleClass('listData_highlight');">
				...
				<td><img src="<?php echo FormatLayer::VideoThumbURL($row['youtube_url']); ?>" /></td>
				<td><?=$row['legenda']?></td>
...
<?
	...
	AdminKernel::showMsgInfo("Não há vídeos cadastrados. Para cadastrar, clique no botão acima.");
}
?>
