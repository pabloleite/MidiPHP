...
$olddir = chdir('../public/images');
foreach(  dir::read('.') as $file )
{
	if( is_file($file) )
	{
		$values['original'] = MySQL::SQLValue($file);
		$db->InsertRow($mainSession, $values);
	}
}
chdir('../../cms');
	...
	<form name="multipleExclusionForm" id="multipleExclusionForm" method="post" enctype="multipart/form-data">
		<table width="100%" border="0" cellpadding="5" cellspacing="0" id="listData">
		  <tr>
			...
			<th><?=AdminKernel::getThOrderLabel("Nome original", "$mainSession.original");?></th>
			<th width="30"> </th>
		  </tr>
		<?
		  $trId = 0;
		...
		?>
			 <tr class="<?="listData_tr".($trId%2)?>" ondblclick="window.location='<?=AdminKernel::getAltHref($row[$primaryKey]);?>';" onmouseover="$(this).toggleClass('listData_highlight');" onmouseout="$(this).toggleClass('listData_highlight');">
				...
				<td>
					<?php
					$img_path = '../userfiles/layout_imagens/'.$row['imagem'];
					if( empty($row['imagem']) || !is_file($img_path) )
						$img_path = '../public/images/'.$row['original'];
					echo AdminKernel::getListDataImage($img_path,80,80);
					?>
				</td>
...
<?
	...
	AdminKernel::showMsgInfo("Não há categorias cadastradas. Para cadastrar, clique no botão acima.");
}
?>
