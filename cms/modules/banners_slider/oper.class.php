...
class Oper {
	...
	private function swap_position($pos)
	{
		global $db;
		
		$rowb = $db->QuerySingleRowArray("SELECT * FROM {$this->mainSession} WHERE {$this->primaryKey} = {$_REQUEST['id']} LIMIT 1");
		$db->UpdateRows($this->mainSession, array('posicao' => $rowb['posicao']), array('posicao' => $pos));
	}
	private function open_position($pos)
	{
		global $db;
		
		$rowb = $db->QuerySingleRowArray("SELECT * FROM {$this->mainSession} WHERE posicao = $pos");
		if( $db->RowCount() )
		{
			$sql = "SELECT COUNT(*) FROM {$this->mainSession}";
			$nbanners = $db->QuerySingleValue($sql);
			$db->UpdateRows($this->mainSession, array('posicao' => MySQL::SQLValue($nbanners)), array($this->primaryKey => $rowb[$this->primaryKey]) );
		}
	}
	...
	private function insert() {
		...
		$posicao = intval($_REQUEST[posicao]);
		$values['ativo'] = MySQL::SQLValue($_REQUEST[ativo]);
		$values['nome'] = MySQL::SQLValue($_REQUEST[nome]);
		$values['link'] = MySQL::SQLValue($_REQUEST[link]);
		$values['posicao'] = MySQL::SQLValue($posicao);
		
		$imagem1 = Utils::upload($_FILES['imagem1'], $config['userfiles_path'].'/'.$this->mainSession."/");
		$values['imagem'] = MySQL::SQLValue($imagem1);
		
		$this->open_position($posicao);
