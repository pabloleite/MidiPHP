...
<form name="oper" id="oper" enctype="multipart/form-data" method="post" onsubmit="return validateForm(this);">
	...
	<div class="field">
		<label for="nome">Posição</label><br />
		<select name="posicao">
			<?php
			$nbanners = $db->QuerySingleValue("SELECT COUNT(*) FROM $mainSession");
			if( $_REQUEST['action']=='inc' )
				++$nbanners;
			for( $n=0;  $n < $nbanners; ++$n )
				printf( '<option value="%d" %s>%d</option>', $n, $alt && $row['posicao']==$n ? 'selected="selected"' : '', $n+1 );
			?>
		</select>
	</div>
